...
class GithubPath(pathlib.PurePosixPath):
  ...
  assert path.path == 'docs/README.md'
  ...
  def iterdir(self) -> List['GithubPath']:
...
def _parse_github_path(path: str) -> Tuple[str, str, str]:
  ...
  root, owner, repo, tree, branch, *sub_path = parts
  ...
  return f'{owner}/{repo}', branch, '/'.join(sub_path)
